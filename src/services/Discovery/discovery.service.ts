...
export class DicoveryService {
    public static ShowDiscovery = async (media: media_type, discovery: paramDiscovery, page: number) => {
        let dataRes: responseSuccess<any>
        let queryParam: string = ''
    
        Object.entries(discovery).forEach(([key,val]) => { queryParam+= this.optionnalDiscoveryParam(key,val)}); 
     ...
     private static optionnalDiscoveryParam = (att: string, data: any): string => {
        switch (att) {
    ...
    public static MovieTrend = async (mediaType?: media_type, time_window?: time_window) : Promise<responseSuccess<any>> => {
        let dataRes: responseSuccess<any>
